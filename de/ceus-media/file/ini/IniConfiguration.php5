...
class IniConfiguration extends ADT_OptionObject
{
	 ...
	 *	@return		string
	 */
	function setConfigValue( $key, $value, $section = false )
	{
		if( $section )
...
//		touch( $filename, filemtime( $this->getOption( 'pathConfig' ).$this->_filename ) );
	...
	function _readIni()
	{
		$ir		= new IniReader( $this->getOption( 'pathConfig' ).$this->_filename, true );

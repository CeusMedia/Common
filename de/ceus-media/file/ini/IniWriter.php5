...
class IniWriter extends IniReader
{
	...
	private function buildLine ($key, $value, $comment)
	{
		...
		if ($keyBreaks <1) $keyBreaks = 1;
		if ($valueBreaks <1) $valueBreaks = 1;
		if ($comment)
	...
	{
		if ($section && !in_array ($section, $this->sections)) $this->addSection ($section); 
				...
				if ($this->usesSections () && eregi ($this->sectionPattern, $line))
				{
					...
					if ($lastSection)
					{
						foreach ($this->added as $property)
						{
							...
							{
								$line = $this->buildLine ($property['key'], $property['value'], $property['comment']).$line;
							}

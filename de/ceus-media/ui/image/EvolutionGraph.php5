...
//		'color_text'			=> array( 0x0, 0x00, 0x00 ),						//  Color of Text
	...
	/*	@var	array		_graphs		Array of Values of one or more Graphs */
	var $_graphs	= array();
	 ...
	 *	@return		void
	 */
	function addGraph( $legend, $color, $data )
	{
		$position	= count( $this->_graphs );
		$this->_graphs[$position]	= array(
			'legend'	=> $legend,
			'color'	=> $color,
			'values'	=> $data,
			);
	...
	{
// the actual stuff that'll make the image
...
// each section uses $i as a local counter (0 based!)
// remember to change it as you move from one section to the next

		// set the image size
		$im = @ImageCreate( $this->getOption( 'width' ), $this->getOption( 'height' ) );
		extract( $this->getOptions() );
		$imageColorBackground	= $this->_setColor( $im, $this->getOption( "color_background" ) );			//  set Background Color
		$imageColorBars		= $this->_setColor( $im, $this->getOption( "color_bars" ) );				//  set Color of Y-Axis Labels
		$imageColorDash		= $this->_setColor( $im, $this->getOption( "color_dash" ) );				//  set Color of dashed Lines
//		$imageColorText		= $this->_setColor( $im, $this->getOption( "color_text" ) );				//  set Color of Text
		$imageColorTitle		= $this->_setColor( $im, $this->getOption( "color_title" ) );					//  set Color of Title Text
		if( $this->getOption( 'transparent' ) )
			ImageColorTransparent( $im, $imageColorBackground );									//  set Background Transparency
		$this->_drawOutlines( $im, $imageColorBars );												//  draw Outlines of Graph Image
		// in case no maximum scale has been provided, calculate the maximum value reached by any of the lines
		if( !isset( $maxValue ) )
		{
			$maxValue	= 0;
			for( $g=0; $g<count( $this->_graphs ); $g++ )
				$maxValue	= max( $maxValue, max( $this->_graphs[$g]['values'] ) );
	//		if( isset( $maxAdjust ) )
	//			$maxValue	+= $maxAdjust; // so that it doesn't touch the upper margin
		...
		$ratio	= floor( $this->getOption( 'width' ) - $this->getOption( 'padding_left' ) - $this->getOption( 'padding_right' ) ) / ( count( $this->_labels ) - 1 );
		for( $i=0; $i<count( $this->_labels ); $i++ )
		{
			if( $i<count( $this->_labels ) -2 )
			{
			...
			ImageString( $im, 1, $width + $this->getOption( 'label_adjust_x' ), $this->getOption( 'height' ) + $this->getOption( 'label_adjust_y' ), $this->_labels[$i], $imageColorTitle );
		}
		// actually output the graphs
		$this->_drawGraphs( $im, $maxValue, $ratio );
	...
	//  --  PRIVATE METHODS  --  //
	function _drawOutlines( &$image, $color )
	{
	...
	function _setColor( &$image, $values )
	{
